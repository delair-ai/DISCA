...
class Trainer:
    ...
    def _save_net(
        ...
        if not temp:
            name = name.replace(".pth", ".pt")
            self.save_to_jit(name)
            print("Saved network: ", name)
        return name

    def load_weights(self, path_weights: str, is_jit=False, verbose=True) -> None:
        """Only to infer (doesn't load scheduler and optimizer state)."""
        if verbose:
            print(f"Load: {path_weights} on {self.device}")
        if is_jit:
            try:
                self.net = torch.jit.load(path_weights, map_location=self.device)
            except:
